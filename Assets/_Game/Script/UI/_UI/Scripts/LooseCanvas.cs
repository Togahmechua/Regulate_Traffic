...
public class LooseCanvas : UICanvas
{
    ...
    public void GetMoney()
    {
        if (!isDone)
        {
            ...
            earnedPoop = PlayerPrefs.GetInt("Poop");
            PoopTxt.text = /*earnedPoop <= 9 ? "0" + earnedPoop.ToString() : */earnedPoop.ToString();
            UIParticleAttractor_OnAttract(PoopTxt);
        }
    }
    ...
    public void UIParticleAttractor_OnAttract(TextMeshProUGUI scoreText)
    {
        scoreText.GetComponent<Animator>().Play(CacheString.TAG_POP);
    }
    #endregion
}